class SocketSpamClassifier {
    ...
    handleUploadSuccess(data) {
        ...
        // Hiển thị biểu đồ phân phối nhãn nếu có
        if (data.distribution_chart) {
            this.displayDistributionChart(data.distribution_chart);
        }
        
        // Hiển thị phân tích độ dài văn bản nếu có
        if (data.text_length_analysis) {
            this.displayTextLengthAnalysis(data.text_length_analysis);
        }
    ...
    displayDistributionChart(chartImage) {
        const distributionSection = document.getElementById('distributionSection');
        const distributionChart = document.getElementById('distributionChart');
        const distributionAnalysis = document.getElementById('distributionAnalysis');
        
        if (distributionSection && distributionChart && distributionAnalysis) {
            distributionChart.src = chartImage;
            distributionChart.classList.remove('hidden');
            
            distributionAnalysis.innerHTML = `
                <i class="fas fa-info-circle"></i> 
                Biểu đồ thể hiện sự phân bố giữa tin nhắn hợp lệ (Ham) và tin nhắn rác (Spam). 
                Giúp xác định vấn đề mất cân bằng lớp trong dữ liệu.
            `;
    ...
    displayTextLengthAnalysis(analysisData) {
        const textLengthSection = document.getElementById('textLengthSection');
        const textLengthStats = document.getElementById('textLengthStats');
        const textLengthChart = document.getElementById('textLengthChart');
        const textLengthInsight = document.getElementById('textLengthInsight');
        
        if (textLengthSection && textLengthStats && textLengthChart && textLengthInsight) {
            const hamStats = analysisData.ham_stats;
            const spamStats = analysisData.spam_stats;
            
            textLengthStats.innerHTML = `
                <div class="stats-column">
                    <h5>Ham (Hợp lệ)</h5>
                    <div class="stat-row">
                        <span class="stat-label">Trung bình:</span>
                        <span class="stat-value">${hamStats.mean.toFixed(1)} ký tự</span>
                    </div>
                    <div class="stat-row">
                        <span class="stat-label">Trung vị:</span>
                        <span class="stat-value">${hamStats.median.toFixed(1)} ký tự</span>
                    </div>
                    <div class="stat-row">
                        <span class="stat-label">Min - Max:</span>
                        <span class="stat-value">${hamStats.min} - ${hamStats.max} ký tự</span>
                    </div>
                </div>
                <div class="stats-column">
                    <h5>Spam (Rác)</h5>
                    <div class="stat-row">
                        <span class="stat-label">Trung bình:</span>
                        <span class="stat-value">${spamStats.mean.toFixed(1)} ký tự</span>
                    </div>
                    <div class="stat-row">
                        <span class="stat-label">Trung vị:</span>
                        <span class="stat-value">${spamStats.median.toFixed(1)} ký tự</span>
                    </div>
                    <div class="stat-row">
                        <span class="stat-label">Min - Max:</span>
                        <span class="stat-value">${spamStats.min} - ${spamStats.max} ký tự</span>
                    </div>
                </div>
            `;
            
            textLengthChart.src = analysisData.chart;
            textLengthChart.classList.remove('hidden');
            
            const difference = Math.abs(spamStats.mean - hamStats.mean);
            const longerType = spamStats.mean > hamStats.mean ? 'spam' : 'ham';
            const shorterType = spamStats.mean > hamStats.mean ? 'ham' : 'spam';
            
            textLengthInsight.innerHTML = `
                <i class="fas fa-lightbulb"></i>
                <strong>Nhận xét:</strong> 
                Tin nhắn ${longerType} có xu hướng dài hơn tin nhắn ${shorterType} (${difference.toFixed(1)} ký tự).
            `;
            
            textLengthSection.classList.remove('hidden');
        }
    }
    ...
    resetUpload() {
        ...
        // Ẩn biểu đồ phân phối
        const distributionSection = document.getElementById('distributionSection');
        if (distributionSection) {
            distributionSection.classList.add('hidden');
        }
        
        // Ẩn phân tích độ dài văn bản
        const textLengthSection = document.getElementById('textLengthSection');
        if (textLengthSection) {
            textLengthSection.classList.add('hidden');
        }
    ...
    handleProcessCompleted(data) {
        ...
        // Bổ sung: Gửi lại dữ liệu cuộc thi nếu nó được chứa trong gói dữ liệu cuối cùng
        if (data.competition_data) {
            this.handleTestFilesReady(data.competition_data);
        }
    ...
    downloadTestFile() {
        if (this.competitionData && this.competitionData.test_csv) {
    ...
    downloadSubmissionFile() {
        if (this.competitionData && this.competitionData.submission_csv) {
